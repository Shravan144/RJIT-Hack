...
const CATEGORY_BADGE = {
  ...
  equipment:   'bg-brand-elevated border-brand-border text-brand-muted',
  other:       'bg-brand-elevated border-brand-border text-brand-muted',
};
...
export default function ProductCard({ product, price, inStock }) {
  ...
  return (
    <div id={`product-card-${product.id}`}
      className="flex flex-col gap-2.5 bg-gradient-card border border-brand-subtle rounded-2xl p-4 hover:-translate-y-0.5 hover:shadow-lg transition-all duration-300">
      ...
      <div className="flex justify-center py-3">
        <Package size={32} className="text-brand-muted" />
      </div>

      <h4 className="font-bold text-brand-base text-sm leading-snug">{product.name}</h4>
      <p className="text-brand-muted text-xs">{product.brand}</p>

      {product.active_ingredients && (
        <p className="text-brand-muted text-[11px] line-clamp-2">{t('components.ai')}: {product.active_ingredients}</p>
      )}

      <div className="flex flex-wrap gap-2 justify-between items-center pt-2 border-t border-brand-subtle mt-auto">
        {price && (
        ...
        {product.registration_number && (
          <div className="text-[11px] text-brand-muted">{t('components.reg')}: {product.registration_number}</div>
        )}
      </div>
    </div>
  );
