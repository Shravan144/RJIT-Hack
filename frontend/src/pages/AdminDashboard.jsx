...
export default function AdminDashboard() {
  ...
  const statCards = [
    { icon: <Users size={22} />, label: 'Total Dealers',   value: stats.totalDealers,   color: 'text-blue-400',   bg: 'bg-blue-400/10   border-blue-400/25'   },
    { icon: <FileText size={22}/>, label: 'Total Reports',  value: stats.totalReports,   color: 'text-purple-400', bg: 'bg-purple-400/10 border-purple-400/25' },
    { icon: <AlertTriangle size={22}/>, label: 'Pending',   value: stats.pendingReports, color: 'text-amber-400',  bg: 'bg-amber-400/10  border-amber-400/25'  },
    { icon: <ShieldCheck size={22}/>, label: 'Verified',    value: stats.verifiedReports,color: 'text-green-400',  bg: 'bg-green-400/10  border-green-400/25'  },
  ];
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10 pb-20">
      <div className="mb-8">
        <h1 className="font-display font-black text-4xl text-slate-100 mb-2">Admin Dashboard</h1>
        <p className="text-slate-400">Manage reports, dealers, and platform trust scores.</p>
      </div>
      ...
      <div className="flex gap-1 border-b border-[hsl(220,14%,20%)] mb-6">
        {[['reports', `Reports (${reports.length})`], ['dealers', `Dealers (${dealers.length})`]].map(([key, label]) => (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[hsl(220,14%,16%)] border-b border-[hsl(220,14%,20%)]">
                {['Shop', 'License', 'Status', 'Trust Score', 'Reports'].map(h => (
